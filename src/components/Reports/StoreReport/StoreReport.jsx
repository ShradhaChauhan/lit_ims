...
const StoreReport = () => {
  ...
  const handleSearch = async () => {
    if (!searchQuery.trim()) {
      toast.warning('Please enter a batch number or item code');
      return;
    }

    try {
      let itemCode = searchQuery;
      if (searchQuery.length > 30) {
        // Extract itemCode from batchNo
        itemCode = searchQuery.substring(7, 15);
      }
    ...
    <div>
      ...
      <nav className="navbar bg-light border-body" data-bs-theme="light">
        <div className="container-fluid">
          <div className="mt-4">
            <h3 className="nav-header header-style">Store Report</h3>
            <p className="breadcrumb">
              <Link to="/dashboard">
                <i className="fas fa-home text-8"></i>
              </Link>{' '}
              <span className="ms-1 mt-1 text-small-gray">/ Store Report</span>
            </p>
          </div>
        </div>
      </nav>
        ...
        <div className="search-box text-8 d-flex">
          <div className="position-relative flex-grow-1">
            ...
            <input
              ...
              placeholder="Enter batch number or item code..."
              value={searchQuery}
              ...
              {results.length === 0 ? (
                ...
                results.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="ps-4">{item.location}</td>
